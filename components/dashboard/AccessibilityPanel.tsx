...
import { Accessibility, Mic, MicOff, ChevronDown } from "lucide-react";
...
export function AccessibilityPanel() {
  ...
  // Collapsed by default to keep the sidebar compact; every control is one
  // tap away via the chevron, and nothing is removed when collapsed.
  ...
  return (
    ...
    >
      {/* Label — always visible, toggles the panel */}
      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        className="flex items-center justify-between gap-2 w-full"
        aria-expanded={expanded}
        aria-controls="accessibility-panel-body"
      >
        ...
        <ChevronDown
          size={13}
          className={cn("text-[#64748B] transition-transform duration-200", expanded && "rotate-180")}
          aria-hidden="true"
        />
      </button>

      {expanded && (
        <div id="accessibility-panel-body" className="mt-3">
          {/* Text size */}
          <div className="mb-3">
