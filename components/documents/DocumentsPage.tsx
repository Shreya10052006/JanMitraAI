  ...
  return (
    <div className="overflow-y-auto h-full" id="main-content">
      <div className="px-6 py-5 space-y-5 max-w-[1280px]">
        ...
        {uploadSuccess && (
          <div className="flex items-center gap-2 px-4 py-3 bg-green-50 rounded-xl border border-green-100 text-sm text-green-700" role="status" aria-live="polite">
          ...
          {[
            ...
            <button key={title}
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[#6B7280] mt-1 leading-relaxed">{sub}</p>
                <div className="mt-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
          ...
          <div className="space-y-5">
            ...
            <section className="bg-white rounded-2xl border border-[#E8E4F8] overflow-hidden" aria-labelledby="ai-rec-heading">
              <div className="px-5 py-4 border-b border-[#F3F0FF]">
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-2">
                    <span className="px-3 py-1.5 rounded-lg bg-[#F3F0FF] text-xs font-semibold text-[#6B3FFF]">Service Selected</span>
                    <span className="text-sm font-semibold text-[#1A1340]">{selectedService.label}</span>
                  </div>
                  <div className="relative">
                    <button
                      onClick={() => setServiceMenuOpen((v) => !v)}
                      className="px-3 py-1.5 rounded-xl border border-[#E8E4F8] text-xs font-medium text-[#374151] hover:bg-[#F9F8FF] transition-colors"
                    ...
                    {serviceMenuOpen && (
                      <ul className="absolute right-0 top-full mt-1 bg-white border border-[#E8E4F8] rounded-xl shadow-lg z-20 overflow-hidden min-w-[220px]" role="listbox">
                        {SERVICES_LIST.map((svc) => (
                          <li key={svc.id}>
                            <button
                              ...
                              className={cn("w-full text-left px-4 py-2.5 text-sm hover:bg-[#F9F8FF] transition-colors", selectedService.id === svc.id && "text-[#6B3FFF] font-semibold bg-[#F3F0FF]")}
                            >
                              {svc.label}
                            </button>
                          </li>
              ...
              {checklist ? (
                <>
                  <div className="grid grid-cols-[1fr_130px_120px] gap-4 px-5 py-2.5 bg-[#F9F8FF] border-b border-[#F3F0FF]">
                  ...
                  <ul className="divide-y divide-[#F9F8FF]" role="list">
                    {checklist.map((doc, i) => (
                      <li key={i}>
                        <div className="grid grid-cols-[1fr_130px_120px] gap-4 items-start px-5 py-3.5 hover:bg-[#F9F8FF] transition-colors">
                          ...
                          <span className={cn("px-2 py-1 rounded-full text-[10px] font-semibold border w-fit mt-1", doc.required ? "bg-red-50 text-red-600 border-red-100" : "bg-gray-50 text-gray-600 border-gray-100")}>
                            {doc.required ? "Required" : "Optional"}
                          </span>
                          <button
                            onClick={() => fileRef.current?.click()}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white hover:opacity-90 transition-all mt-1 w-fit"
                ...
                <>
                  ...
                  <ul className="divide-y divide-[#F9F8FF]" role="list">
                    {RECOMMENDED_DOCS.map((doc) => {
                      ...
                      return (
                        <li key={doc.id}>
                          <div className="grid grid-cols-[1fr_130px_120px] gap-4 items-center px-5 py-3.5 hover:bg-[#F9F8FF] transition-colors">
                            ...
                            <div className={cn("flex items-center gap-1.5 px-2.5 py-1 rounded-full w-fit", s.bg)}>
                              <StatusIcon size={12} style={{ color: s.dot }} aria-hidden="true" />
                              <span className={cn("text-xs font-semibold", s.color)}>{s.label}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              {doc.status === "completed" ? (
                                <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#E8E4F8] text-xs font-medium text-[#374151] hover:bg-[#F9F8FF] transition-colors" aria-label={`View ${doc.name}`}>
                              ...
                              <div className="relative">
                                <button onClick={() => setActiveMenuId(activeMenuId === doc.id ? null : doc.id)}
                                  className="p-1.5 rounded-lg hover:bg-[#F3F0FF] text-[#9CA3AF] transition-colors"
                                  aria-label={`More options for ${doc.name}`} aria-haspopup="true" aria-expanded={activeMenuId === doc.id}>
                                  <MoreVertical size={14} aria-hidden="true" />
                                </button>
                                {activeMenuId === doc.id && (
                                  <div className="absolute right-0 top-full mt-1 w-36 bg-white border border-[#E8E4F8] rounded-xl shadow-lg z-20 overflow-hidden" role="menu">
                                    {["View Details", "Download", "Delete"].map((opt) => (
                                      <button key={opt} role="menuitem" onClick={() => setActiveMenuId(null)}
                                        className={cn("w-full text-left px-3 py-2 text-xs hover:bg-[#F9F8FF] transition-colors", opt === "Delete" ? "text-[#EF4444]" : "text-[#374151]")}>
                                        {opt}
                                      </button>
                                    ))}
                                  </div>
              ...
              <div className="px-5 py-3 border-t border-[#F3F0FF] flex justify-center">
                <button className="flex items-center gap-2 px-5 py-2 rounded-xl border border-[#6B3FFF]/30 text-sm font-semibold text-[#6B3FFF] hover:bg-[#F3F0FF] transition-all" aria-label="View full document list">
            ...
            <section aria-labelledby="recent-docs-heading">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {filteredDocs.slice(0, 8).map((doc) => (
                  ...
                  >
                    {"isLocal" in doc && (
                      <span className="absolute top-2 right-2 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-blue-50 text-blue-600 border border-blue-100">NEW</span>
                    )}
                    ...
                    <p className="text-[10px] text-[#9CA3AF] mt-0.5">{doc.type} · {doc.size}</p>
                    <p className="text-[10px] text-[#9CA3AF]">{doc.date}</p>
                    <div className={cn("mt-2 text-center py-0.5 rounded-full text-[10px] font-semibold border", DOC_STATUS_BADGE[doc.status])}>
                      {doc.status}
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-4">
            ...
            <section className="bg-white rounded-2xl border border-[#E8E4F8] p-5" aria-labelledby="checklist-heading">
              ...
              <div className="flex items-start gap-2 bg-green-50 rounded-xl px-3 py-2.5 border border-green-100">
                <CheckCircle2 size={14} className="text-[#10B981] flex-shrink-0 mt-0.5" aria-hidden="true" />
              ...
              <div className="space-y-2">
                {QUICK_LINKS.map((link) => (
                  <a key={link.id} href={link.href} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#F9F8FF] border border-transparent hover:border-[#E8E4F8] transition-all group"
